...
- テーブル分割は大きく分けると以下の2つ
  - **水平分割**: レコードを分割する
    - ex: 履歴テーブルを年度ごとに分割する
  - **垂直分割**: カラムを分割する
    - ex: 顧客情報テーブルを、基本情報テーブルと詳細情報テーブルに分割する
- どちらも、極力採用しない方が良い
  - **水平分割**
    - [ 分割する意味的な理由がない ] 論理的には意図がない。パフォーマンスの観点のみでの分割になる
    - [ 拡張性に乏しい ] テーブルが増えるたびに、アプリケーション側のコードの改修が必要
    - → **「パーティション」を使った方が良い**
  - **垂直分割**:
    - [ 分割する意味的な理由がない ] 論理的には意図がない。パフォーマンスの観点のみでの分割になる
- 代替案
  - パーティション
    - テーブルのデータ構造を論理的に変更せず、物理的に格納する場所を変更する手法
    - メリット : 元のテーブルのデータ構造を変更せず、SQLがアクセスするデータ量を1/nに減らすことができる
    - ※ 但し、パーティションはRDBMSによってはサポートされていないものもあるので、採用する際は注意が必要
    - **※ インデックスの方を先に検討した方が良い**
  - データマート(列の絞り込み)
    - 任意のテーブルの特定の列のみを抽出して、テーブルを作成する方法
    - メリット : 元のテーブルのデータ構造を変更せず、SQLがアクセスするデータ量を減らすことができる
    - デメリット : データマートとオリジナルのテーブルをいつ同期するかを考慮する必要がある
      - 多くは1回/日 のバッチ処理だが、システムの要件による
  - サマリーテーブル
    - `SUM`や`AVG`など、集約関数を用いて取得したデータをあらかじめ、別テーブルに保存しておく方法
    - メリット : 集計処理を事前に行っておくことで、アプリケーション側の処理を軽減できる
    - デメリット : データマートと同様、同期処理が必要

### ダブルマスタ
- マスターテーブルが複数存在すること
- 実際のマスタデータを取得するのに、テーブルを完全外部結合で取得す必要がある。
  - パフォーマンスも悪いし、データ構造が違うときには、データの取得が難しい
- システムの統廃合で起きがちな問題
- [ 結論 ] マスターは一つのテーブルにする！
  - 移行作業はめんどくさいが、後のコストに大きく関わる

***

## 第8章 論理設計のグレーノウハウ
アンチパターンになるかならないか、ギリギリの設計を紹介(7章と同じく、当たり前だったり新しく得られた知見がないやつは割愛)

### 主キーがない or 主キーを設定しているが不十分
- **【そもそも論】妥当な主キーがないのは、ちゃんとデータクレンジングがされていない証拠**
- 主キーはあるが、使いまわせれている場合
  - 任意のデータが不要になった場合、他のデータにその主キーを使いまわしているケース
  - 以下の対策が考えられる
    - [代理キー(サロゲートキー)]
      - テーブルのカラムとは全く関係のないidカラムを作成し、そのカラムを主キーとする
      - RailsのActiveRecordや、PrismaなどのORMで使われている手法
      - 書籍ではアンチパターンとして賞愛されているが、近年ではよくあるやり方
      - 「主キーを作成するためだけのキーなので、他の開発者が見たら、このカラムなに？？？」ってなるからアンチパターンだよってされてる
      - ActiveRecordなどのORMでは基本的にどのテーブルでもidカラムがあるので、その誤解を招くことはない = アンチパターンではない(アンチパターンとされる原因を払拭できている)
    - [タイムスタンプ]
      - 「いつまではこのデータは有効だよ〜」っていう日付をカラムとして持つ
      - その期限が過ぎれば再度、有効期限を新たに設定したデータを作成する
      - メリット
        - SQLを使う際にクエリがシンプルになる
      - デメリット
        - データ量が増える
    - [インターバル]
      - データの有効期間を開始日時と終了日時で管理する
      - 終了日時は、カラムが持てる最大値を設定するとSQLでカラムが空(`NULL`)である場合を考慮する必要がなく楽
      - メリット
        - データ量が増えない
      - デメリット
        - SQLが複雑になる
  - 個人的にはサロゲートキーが一番使いやすいと思う

### アドホック(場当たり的)な集計キー
- データを集計用で特定のコードを割り当て分類する際の方法
- ❌ テーブルに集計用のカラムを追加する
  - 短いスパンで集計するコード体系が変わると、その分データを更新する必要がある
  - 場当たり的なカラム追加の文化が根付き、同じようなケースでどんどんカラムが増えてしまう
- ✅ 集計用のカラムを別テーブルに切り出す
  - メリット
    - 集計と元データを分離することで、テーブルの責務を明確にできる
  - デメリット
    - 集計の際にJOINが必要になるため、パフォーマンスが悪化する
  - (個人的には) 今後もデータの変更がなさそうなケースに関しては、元テーブルにカラムを追加するのもありだと思う。集計用のカラムが複数必要になれば別テーブルに切り出す
    - ex: 都道府県に対して、地方コードを付与する場合
- ✅ ビューを使う
  - メリット
    - カラムを追加する必要がないので、パフォーマンスが悪化しない
  - デメリット
    - 多段ビュー(後述)にならないように注意が必要

### 多段ビュー
- **ビューのネストはアンチパターン！！！！！！！**
- **ビューはあくまでも、SELECT文を保存するだけ！！！(SELECT文の隠蔽)**
- 実際にやっていることは、ただのSELECT文であることを忘れない！！！
  - 実際には、①ビューに設定されてリウSELECT文と、②ビューに対して投げるSQLからRDBが最適なSQLを生成する
- ビューがさらに別のビューを指定するすると開発者が実際にどのようなSQLが生成されるかわかりにくくなる
...
### そもそも論
- **RDBで木構造を扱うのは、得意ではない**
- RDBは表(二次元)であり、それで立体的である木構造を扱うのは苦手分野
だが、表現できる方法はある
- **それぞれのメリデメを検討し、適切な方法を選択することが重要**

### 比較表
- 詳細は割愛。以下の表を参照して、選定時により細かく調べる。
- 個人的には「やっぱりRDBで木構造を扱うのはアンチパターンなのかな〜」って感じ
  - NoSQLの方が向いている気がする(あんま知らんけど)
  - 木構造のやつはFireStoreとかで保存して、RDBにはそのパスを保存するとかの方がいいのかな〜って思う(画像をRDBに入れずにS3に保存して、そのパスをRDBに保存する的なノリ)

| 方法 | 検索処理 | 更新処理 | 感想 |
| :---: | --- | --- | --- |
| 隣接リストモデル | ❌ | ❌ | 使わない方が良い |
| 入れ子集合モデル | ⭕️ | ❌ (パフォーマンス悪い) | 入れ子区間モデルが上位互換 |
| 入れ子区間モデル | ⭕️ | ⭕️ ( ※ 但し実数型の有効桁数に依存するので注意 ) | 実数型の範囲に収まるケースならアリ |
| 経路列挙モデル | ⭕️ | ❌ | 更新がほぼないケースなら、こっちの方が直感的で分かりやすいのでアリ |
